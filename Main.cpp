...
int main()
{
	...
	for (int i = 0; i < textToAnalyze.length(); ++i)
	{
		if (textToAnalyze[i] == 'a' || textToAnalyze[i] == 'e' || textToAnalyze[i] == 'i' ||
			...
			textToAnalyze[i] == 'U')
		{
			vowels++; //found a bug, was taking values away from vowel count, fixed by adding them
		}
		else if ((textToAnalyze[i] >= 'a'&& textToAnalyze[i] <= 'z') || (textToAnalyze[i] >= 'A'&& textToAnalyze[i] <= 'Z'))
		{
			++consonants; // found a bug, consonants were commented out so they weren't being counted
		}
